...
lr_result1 <- evaluate_model_fit(thr, ground_truth, lr_probs1$.pred_1)
...
lr_result2 <- evaluate_model_fit(thr, ground_truth, lr_probs2$.pred_1)
...
thresh<- threshold_info$threshold

###--------------Apply to test data

rf_probs_test <- predict(rf_fit, dat_test, type = "prob")$.pred_1

rf_result <- evaluate_model_fit(thresh, ground_truth, rf_probs_test)
...
xgb_result <- evaluate_model_fit(thresh, ground_truth, xgb_probs_test)
...
The positive predictive value (PPV) is the probability that an individual predicted to have stroke truly has stroke, while the negative predictive value (NPV) is the probability that an individual predicted not to have stroke truly does not have stroke. Considering our logistic regression model as a diagnostic tool, the probability that an individual predicted to have stroke has stroke is 16% and the probability that an individual predicted not to have stroke does not have stroke is 98%.
